...
function IllustrationStripeFrame({ children }: { children: ReactNode }) {
  return (
    <div className="inline-block text-[0] leading-[0]">
      <div className="relative m-0 box-border inline-block p-0">{children}</div>
    </div>
  )
}
...
export default function BizLoginHubClient({ useHeroPhoto }: { useHeroPhoto: boolean }) {
          ...
          >
            <div className="mx-auto w-full max-w-[min(100%,560px)]">
              ...
              <p className="mt-5 max-w-xl text-[0.9375rem] font-normal leading-[1.267] lg:mt-7">
                {b.introBeforeSuite}
                <strong className="font-bold">{b.suiteProduct}</strong>
                {b.introBetween}
                <strong className="font-bold">{b.adsProduct}</strong>
                {b.introAfter}
              </p>
